...
class BaseResult
  attr_reader :resource, :errors, :success, :metadata, :cart, :order, :error_type

  def initialize(success:, resource: nil, errors: [], cart: nil, order: nil, error_type: nil, **metadata)
    @success = success
    @resource = resource
    @errors = Array(errors)
    @metadata = metadata
